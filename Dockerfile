...
# Cоздаем пустой каталог с именем source на корневом уровне проекта.
...
# команда для выполнения внутри контейнера: с помощью Python запускаем runserver (development-сервера
# для demo будет достаточно) и укажем слушать все интерфейсы, чтобы сервер был доступен снаружи.
